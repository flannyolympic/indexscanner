...
# --- VERSION 1.6.2 LIVE DATA HOTFIX ---
APP_VERSION = "v1.6.2 Hotfix" 
...
# --- SEPARATED UNIVERSES (CRITICAL FIX) ---
STOCKS = [
...
# --- REPAIRED DATA FETCHING (NATIVE) ---
def get_market_data(ticker, retries=3):
    """
    Revised to use native yfinance handling. Removed manual session injection
    that was causing 403 Forbidden errors.
    """
    attempt = 0
    while attempt <= retries:
        try:
            ...
            # Native Call - No Session Injection
    ...
    try:
        ...
        if "quotes" in data:
            for item in data["quotes"]:
                if "symbol" in item:
                    suggestions.append({"symbol": item["symbol"], "name": item.get("shortname", item.get("longname", "")), "exch": item.get("exchDisp", item.get("exchange", ""))})
